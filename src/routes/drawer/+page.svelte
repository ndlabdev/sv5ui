...
<div class="space-y-8">
	...
	<section class="space-y-3">
		...
		<div class="rounded-lg bg-surface-container-high p-4">
			...
			>
				...
				{#snippet body()}
					<p class="text-on-surface-variant">
						Slides up from the bottom by default. Swipe down or click the overlay to close.
					</p>
				{/snippet}
				{#snippet footer()}
	...
	<section class="space-y-3">
		...
		<p class="text-sm text-on-surface-variant">
			Use the <code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">direction</code> prop
			to open from any edge.
		</p>
		<div class="flex flex-wrap gap-3 rounded-lg bg-surface-container-high p-4">
			{#each directions as dir}
				...
				>
					...
					{/snippet}
					{#snippet footer()}
						<Button label="Close" variant="outline" onclick={() => (directionOpen[dir] = false)} />
					{/snippet}
				</Drawer>
			{/each}
		</div>
	...
	<section class="space-y-3">
		...
		<div class="flex flex-wrap gap-3 rounded-lg bg-surface-container-high p-4">
			<Drawer bind:open={insetOpen} inset title="Inset Drawer" description="Rounded, offset from edges.">
				<Button variant="outline" label="Bottom Inset" />
				{#snippet body()}
					<p class="text-on-surface-variant">This drawer floats with rounded corners on all sides.</p>
				{/snippet}
				{#snippet footer()}
			...
			{#each directions as dir}
				...
				>
					<Button variant="soft" size="sm" label="Inset {dir.charAt(0).toUpperCase() + dir.slice(1)}" />
					{#snippet body()}
						<p class="text-on-surface-variant">Inset mode combined with <strong>{dir}</strong> direction.</p>
					{/snippet}
					{#snippet footer()}
						<Button label="Close" variant="outline" onclick={() => (insetDirectionOpen[dir] = false)} />
					{/snippet}
				</Drawer>
			{/each}
		</div>
	...
	<section class="space-y-3">
		...
		<div class="flex flex-wrap gap-3 rounded-lg bg-surface-container-high p-4">
			<Drawer bind:open={noHandleOpen} handle={false} title="No Handle" description="Handle is hidden.">
				...
				{/snippet}
				{#snippet footer()}
					<Button label="Close" variant="outline" onclick={() => (noHandleOpen = false)} />
				{/snippet}
			</Drawer>
					...
					<div class="mt-3 space-y-2">
						{#each Array(5) as _, i (i)}
							<div class="rounded-md bg-surface-container p-3">
								<p class="text-sm text-on-surface-variant">Scrollable item {i + 1}</p>
							</div>
						{/each}
					</div>
				{/snippet}
				{#snippet footer()}
					<Button label="Close" variant="outline" onclick={() => (handleOnlyOpen = false)} />
				{/snippet}
			</Drawer>
		</div>
	</section>
			...
			>
				...
				{#snippet body()}
					<p class="text-on-surface-variant">
						...
						Requires a <code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">[data-vaul-drawer-wrapper]</code> attribute on the app container.
					</p>
				{/snippet}
				{#snippet footer()}
					<Button label="Close" variant="outline" onclick={() => (scaleBackgroundOpen = false)} />
				{/snippet}
			</Drawer>
		</div>
	</section>
	...
	<section class="space-y-3">
		...
		<p class="text-sm text-on-surface-variant">
			<code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">closeThreshold</code> (0–1)
			controls how far the user must swipe before the drawer closes. Default is 0.25.
		</p>
		<div class="flex flex-wrap gap-3 rounded-lg bg-surface-container-high p-4">
			<Drawer
			...
			>
				...
				{#snippet body()}
					<p class="text-on-surface-variant">
						You need to swipe 80% of the drawer height before it will close.
						Short swipes will snap back.
					</p>
				{/snippet}
				{#snippet footer()}
					<Button label="Close" variant="outline" onclick={() => (closeThresholdOpen = false)} />
				{/snippet}
			</Drawer>
		</div>
	</section>
	...
	<section class="space-y-3">
		...
		<p class="text-sm text-on-surface-variant">
			<code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">onDrag</code> and
			<code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">onRelease</code> let
			you track the drawer's drag state in real-time.
		</p>
		<div class="rounded-lg bg-surface-container-high p-4">
			<div class="mb-3">
			...
			>
				...
				{#snippet body()}
					<p class="text-on-surface-variant">
						Drag the drawer and watch the badge update in real-time.
						Current drag: <strong>{Math.round(dragPercent * 100)}%</strong>
					</p>
				{/snippet}
				{#snippet footer()}
					<Button label="Close" variant="outline" onclick={() => (dragTrackOpen = false)} />
				{/snippet}
			</Drawer>
		</div>
	</section>
	...
	<section class="space-y-3">
		...
		<p class="text-sm text-on-surface-variant">
			<code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">scrollLockTimeout</code>
			...
			>
				...
				{#snippet body()}
					<div class="max-h-48 space-y-2 overflow-y-auto">
						<p class="text-on-surface-variant">Scroll this list, then try dragging the drawer immediately:</p>
				...
				{/snippet}
				{#snippet footer()}
					<Button label="Close" variant="outline" onclick={() => (scrollLockOpen = false)} />
				{/snippet}
			</Drawer>
		</div>
	</section>
		...
		<div class="flex flex-wrap gap-3 rounded-lg bg-surface-container-high p-4">
			...
			>
				...
				{#snippet body()}
					<div class="space-y-2">
						<p class="text-on-surface-variant">
							Drag between 25%, 50%, and 100% heights. Overlay fades from the 2nd snap point.
						</p>
			...
			>
				...
				{#snippet body()}
					<div class="space-y-2">
						<p class="text-on-surface-variant">
							With <code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">snapToSequentialPoint</code>,
							fast swipes won't skip snap points. Each point must be visited in order.
						</p>
	...
	<section class="space-y-3">
		...
		<div class="rounded-lg bg-surface-container-high p-4">
			...
			>
				...
				{#snippet body()}
					<p class="text-on-surface-variant">Click below to open a child drawer on top of this one.</p>
					<div class="mt-3">
						<Drawer
						...
						>
							...
							{/snippet}
							{#snippet footer()}
								<Button label="Close Inner" variant="outline" onclick={() => (nestedInnerOpen = false)} />
							{/snippet}
						</Drawer>
					</div>
				{/snippet}
				{#snippet footer()}
					<Button label="Close" variant="outline" onclick={() => (nestedOuterOpen = false)} />
				{/snippet}
			</Drawer>
		</div>
	</section>
			...
			<Drawer bind:open={slotsOpen}>
				...
				{#snippet body()}
					<p class="text-on-surface-variant">
						...
						replaces the default title + description with fully custom markup.
					</p>
				{/snippet}
				{#snippet footer()}
					<div class="flex gap-2">
						<Button label="Cancel" variant="outline" class="flex-1" onclick={() => (slotsOpen = false)} />
						<Button label="Confirm" variant="solid" color="primary" class="flex-1" onclick={() => (slotsOpen = false)} />
					</div>
				{/snippet}
			</Drawer>
	...
	<section class="space-y-3">
		...
		<p class="text-sm text-on-surface-variant">
			<code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">onOpenChange</code>,
			<code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">onClose</code>, and
			<code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">onAnimationEnd</code>.
		</p>
		<div class="rounded-lg bg-surface-container-high p-4">
			<div class="mb-3 space-y-1">
				{#if callbackLog.length === 0}
					<p class="text-sm text-on-surface-variant italic">Open/close the drawer to see callback logs...</p>
				{/if}
	...
	<section class="space-y-3">
		...
		<div class="rounded-lg bg-surface-container-high p-4">
			...
			>
				...
				{#snippet body()}
					<p class="text-on-surface-variant">
						This drawer is rendered inline in the DOM rather than being portalled to the body.
					</p>
				{/snippet}
				{#snippet footer()}
	...
	<section class="space-y-3">
		...
		<p class="text-sm text-on-surface-variant">
			...
			<code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">open</code> and
			toggling it externally.
		</p>
		<div class="flex gap-3 rounded-lg bg-surface-container-high p-4">
			...
			>
				{#snippet body()}
					<p class="text-on-surface-variant">
						This drawer has no <code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">children</code>
						slot. It was opened by setting <code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">open = true</code> externally.
					</p>
				{/snippet}
				{#snippet footer()}
			...
			>
				...
				{#snippet body()}
					<p class="text-on-primary-container/80">
						This drawer overrides <code class="rounded bg-on-primary-container/10 px-1.5 py-0.5 text-xs text-on-primary-container">content</code>,
						<code class="rounded bg-on-primary-container/10 px-1.5 py-0.5 text-xs text-on-primary-container">title</code>,
						<code class="rounded bg-on-primary-container/10 px-1.5 py-0.5 text-xs text-on-primary-container">description</code>,
						<code class="rounded bg-on-primary-container/10 px-1.5 py-0.5 text-xs text-on-primary-container">handle</code>, and
						<code class="rounded bg-on-primary-container/10 px-1.5 py-0.5 text-xs text-on-primary-container">container</code> slot classes.
					</p>
				{/snippet}
				{#snippet footer()}
	...
	<section class="space-y-3">
		...
		<div class="grid gap-4 sm:grid-cols-2">
			...
			<div class="rounded-lg bg-surface-container-high p-4">
				...
				<Drawer bind:open={settingsOpen} title="Settings">
					...
					{#snippet body()}
						<div class="space-y-4">
							{#each [
								{ icon: 'lucide:user', label: 'Account', desc: 'Manage your profile' },
								{ icon: 'lucide:bell', label: 'Notifications', desc: 'Push, email, SMS' },
								{ icon: 'lucide:shield', label: 'Privacy', desc: 'Data and permissions' },
								{ icon: 'lucide:palette', label: 'Appearance', desc: 'Theme and display' },
								{ icon: 'lucide:globe', label: 'Language', desc: 'English (US)' }
							] as item}
								<button class="flex w-full items-center gap-3 rounded-lg p-2 text-left transition-colors hover:bg-surface-container">
									<div class="flex size-10 items-center justify-center rounded-full bg-surface-container-highest">
										<Icon name={item.icon} size="18" class="text-on-surface-variant" />
									</div>
									<div class="flex-1">
										<p class="text-sm font-medium text-on-surface">{item.label}</p>
										<p class="text-xs text-on-surface-variant">{item.desc}</p>
									</div>
									<Icon name="lucide:chevron-right" size="16" class="text-on-surface-variant" />
								</button>
							{/each}
						</div>
					{/snippet}
			...
			<div class="rounded-lg bg-surface-container-high p-4">
				...
				<Drawer bind:open={notificationsOpen} direction="right">
					...
					{#snippet body()}
						<div class="space-y-3">
							{#each [
								{ title: 'New message', desc: 'John sent you a message', time: '2m ago', unread: true },
								{ title: 'Build completed', desc: 'Pipeline #482 passed', time: '15m ago', unread: true },
								{ title: 'Review requested', desc: 'PR #31 needs review', time: '1h ago', unread: true },
								{ title: 'Deploy finished', desc: 'v2.4.0 deployed to prod', time: '3h ago', unread: false },
								{ title: 'Comment added', desc: 'Alice commented on issue', time: '5h ago', unread: false }
							] as n}
								<div class="flex gap-3 rounded-lg p-2 {n.unread ? 'bg-primary/5' : ''}">
									<div class="mt-0.5 size-2 shrink-0 rounded-full {n.unread ? 'bg-primary' : 'bg-transparent'}"></div>
									<div class="flex-1">
										<p class="text-sm font-medium text-on-surface">{n.title}</p>
										<p class="text-xs text-on-surface-variant">{n.desc}</p>
										<p class="mt-1 text-xs text-on-surface-variant/60">{n.time}</p>
									</div>
								</div>
							{/each}
						</div>
			...
			<div class="rounded-lg bg-surface-container-high p-4">
				...
				>
					<Button variant="outline" leadingIcon="lucide:filter" label="Filters" class="w-full" />
					{#snippet body()}
						<div class="space-y-4">
							<div>
								...
								<div class="flex flex-wrap gap-2">
									{#each ['All', 'Electronics', 'Clothing', 'Books', 'Home'] as cat}
										<Button size="xs" variant={cat === 'All' ? 'solid' : 'outline'} color={cat === 'All' ? 'primary' : 'secondary'} label={cat} />
									{/each}
								</div>
							</div>
							<Separator />
							<div>
								...
								<div class="flex flex-wrap gap-2">
									{#each ['Under $25', '$25-$50', '$50-$100', '$100+'] as price}
										<Button size="xs" variant="outline" color="secondary" label={price} />
									{/each}
								</div>
							</div>
							<Separator />
							<div>
								...
								<div class="flex flex-wrap gap-2">
									{#each ['4+', '3+', '2+', 'Any'] as r}
										<Button size="xs" variant="outline" color="secondary" label="{r} stars" />
									{/each}
								</div>
							</div>
						</div>
					...
					{#snippet footer()}
						<div class="flex gap-2">
							<Button label="Reset" variant="outline" class="flex-1" onclick={() => (filterOpen = false)} />
							<Button label="Apply Filters" variant="solid" color="primary" class="flex-1" onclick={() => (filterOpen = false)} />
						</div>
					{/snippet}
				</Drawer>
			</div>
			...
			<div class="rounded-lg bg-surface-container-high p-4">
				...
				<Drawer
					...
					handle={false}
				>
					<Button variant="outline" leadingIcon="lucide:trash-2" label="Delete Account" color="error" class="w-full" />
					{#snippet content()}
						<div class="flex flex-col items-center gap-4 p-6 text-center">
							...
							<div class="flex w-full gap-2">
								<Button label="Cancel" variant="outline" class="flex-1" onclick={() => (confirmOpen = false)} />
								<Button label="Delete" variant="solid" color="error" class="flex-1" onclick={() => (confirmOpen = false)} />
							</div>
						</div>
					{/snippet}
				</Drawer>
