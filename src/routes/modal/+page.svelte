...
<div class="space-y-8">
	...
	<section class="space-y-3">
		...
		<div class="rounded-lg bg-surface-container-high p-4">
			...
			>
				...
				{#snippet body()}
					<p class="text-on-surface-variant">
						A centered modal dialog. Click the X button, press Escape, or click outside to close.
					</p>
				{/snippet}
				{#snippet footer()}
					<Button label="Cancel" variant="outline" onclick={() => (basicOpen = false)} />
					<Button label="Confirm" variant="solid" color="primary" onclick={() => (basicOpen = false)} />
				{/snippet}
			</Modal>
		</div>
	</section>
	...
	<section class="space-y-3">
		...
		<div class="rounded-lg bg-surface-container-high p-4">
			...
			>
				...
				{#snippet body()}
					<p class="text-on-surface-variant">
						The modal covers the entire viewport. Useful for complex forms, media viewers, or immersive content.
					</p>
				{/snippet}
				{#snippet footer()}
					<Button label="Close" variant="outline" onclick={() => (fullscreenOpen = false)} />
				{/snippet}
			</Modal>
		</div>
	</section>
	...
	<section class="space-y-3">
		...
		<p class="text-sm text-on-surface-variant">
			When <code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">scrollable</code> is
			<code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">true</code>, the entire overlay
			scrolls with the content placed inside it. When
			<code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">false</code> (default), only
		...
		<div class="flex flex-wrap gap-3 rounded-lg bg-surface-container-high p-4">
			...
			>
				...
				{#snippet body()}
					<div class="space-y-3">
						<p class="text-on-surface-variant">
							The entire overlay area is scrollable. The modal scrolls within the viewport.
						</p>
				...
				{/snippet}
				{#snippet footer()}
					<Button label="Close" variant="outline" onclick={() => (scrollableOpen = false)} />
				{/snippet}
			</Modal>
			...
			>
				...
				{/snippet}
				{#snippet footer()}
					<Button label="Close" variant="outline" onclick={() => (scrollableFullscreenOpen = false)} />
				{/snippet}
			</Modal>
		</div>
	</section>
		...
		<div class="rounded-lg bg-surface-container-high p-4">
			<div class="mb-3 space-y-1">
				{#if callbackLog.length === 0}
					<p class="text-sm text-on-surface-variant italic">Open/close the modal to see callback logs...</p>
				{/if}
	...
	<section class="space-y-3">
		...
		<div class="rounded-lg bg-surface-container-high p-4">
			...
			>
				...
				{#snippet body()}
					<p class="text-on-surface-variant">
						This modal is rendered inline in the DOM.
					</p>
				{/snippet}
				{#snippet footer()}
		...
		<div class="grid gap-4 sm:grid-cols-2">
			...
			<div class="rounded-lg bg-surface-container-high p-4">
				...
				<Modal
					...
					close={false}
				>
					<Button variant="outline" leadingIcon="lucide:trash-2" label="Delete Item" color="error" class="w-full" />
					{#snippet content()}
			...
			<div class="rounded-lg bg-surface-container-high p-4">
				...
				>
					<Button variant="outline" leadingIcon="lucide:plus" label="New Project" class="w-full" />
					{#snippet body()}
						<div class="space-y-4">
							<div>
								<label for="name" class="mb-1.5 block text-sm font-medium text-on-surface">Project Name</label>
								<input
									type="text"
									id="name"
									class="w-full rounded-md border border-outline-variant bg-surface px-3 py-2 text-on-surface placeholder:text-on-surface-variant/50 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
									placeholder="My Awesome Project"
								/>
							</div>
							<div>
								<label for="desc" class="mb-1.5 block text-sm font-medium text-on-surface">Description</label>
								<textarea
									id="desc"
									rows="3"
									class="w-full rounded-md border border-outline-variant bg-surface px-3 py-2 text-on-surface placeholder:text-on-surface-variant/50 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
							...
							<div>
								<label for="visibility" class="mb-1.5 block text-sm font-medium text-on-surface">Visibility</label>
								<select
									id="visibility"
									class="w-full rounded-md border border-outline-variant bg-surface px-3 py-2 text-on-surface focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
								>
					...
					{/snippet}
					{#snippet footer()}
						<Button label="Cancel" variant="outline" onclick={() => (formOpen = false)} />
						<Button label="Create Project" variant="solid" color="primary" onclick={() => (formOpen = false)} />
					{/snippet}
				</Modal>
			</div>
			...
			<div class="rounded-lg bg-surface-container-high p-4">
				...
				<Modal bind:open={alertOpen}>
					<Button variant="outline" leadingIcon="lucide:info" label="Show Alert" class="w-full" />
					{#snippet content()}
						<div class="flex flex-col items-center gap-4 p-6 text-center">
							<div class="flex size-14 items-center justify-center rounded-full bg-info/10">
								<Icon name="lucide:info" size="28" class="text-info" />
							</div>
							<h3 class="text-lg font-semibold text-on-surface">New Features Available</h3>
							<p class="text-sm text-on-surface-variant">
								We've added new features to improve your experience. Check out the changelog for more details.
							</p>
							<Button label="Got it!" variant="solid" color="primary" class="w-full" onclick={() => (alertOpen = false)} />
						</div>
					{/snippet}
				</Modal>
			</div>
			...
			<div class="rounded-lg bg-surface-container-high p-4">
				...
				>
					<Button variant="outline" leadingIcon="lucide:scroll-text" label="Terms" class="w-full" />
					{#snippet body()}
						<div class="space-y-4 text-sm text-on-surface-variant">
							{#each Array(15) as _, i (i)}
								<div>
									<h4 class="mb-1 font-medium text-on-surface">Section {i + 1}</h4>
									<p>
					...
					{/snippet}
					{#snippet footer()}
						<Button label="Decline" variant="outline" onclick={() => (imageOpen = false)} />
						<Button label="Accept" variant="solid" color="primary" onclick={() => (imageOpen = false)} />
					{/snippet}
				</Modal>
			</div>
		</div>
