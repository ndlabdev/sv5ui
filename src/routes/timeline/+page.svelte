<script lang="ts">
	...
	const playgroundItems: TimelineItem[] = [
		{ value: 1, icon: 'lucide:file-text', title: 'Step 1', description: 'First step description', date: 'Jan 1' },
		{ value: 2, icon: 'lucide:settings', title: 'Step 2', description: 'Second step description', date: 'Jan 2' },
		{ value: 3, icon: 'lucide:check-circle', title: 'Step 3', description: 'Third step description', date: 'Jan 3' },
		{ value: 4, icon: 'lucide:flag', title: 'Step 4', description: 'Final step description', date: 'Jan 4' }
	]

	// Order tracking demo
	let orderStep = $state(2)
	const orderItems: TimelineItem[] = [
		{ value: 1, icon: 'lucide:shopping-cart', title: 'Order Placed', description: 'Your order has been confirmed', date: 'Dec 20' },
		{ value: 2, icon: 'lucide:package', title: 'Processing', description: 'We are preparing your items', date: 'Dec 21' },
		{ value: 3, icon: 'lucide:truck', title: 'Shipped', description: 'On the way to delivery address', date: 'Dec 22' },
		{ value: 4, icon: 'lucide:home', title: 'Delivered', description: 'Package delivered successfully', date: 'Dec 24' }
	]

	// Activity feed
	const activityItems: TimelineItem[] = [
		{ avatar: { src: 'https://i.pravatar.cc/150?u=john', alt: 'John' }, title: 'John merged PR #142', description: 'feat: add new Timeline component', date: '5 min ago' },
		{ avatar: { src: 'https://i.pravatar.cc/150?u=sarah', alt: 'Sarah' }, title: 'Sarah commented', description: 'Looks great! Ready for review.', date: '15 min ago' },
		{ avatar: { src: 'https://i.pravatar.cc/150?u=mike', alt: 'Mike' }, title: 'Mike pushed 3 commits', description: 'fix: resolve styling issues', date: '1 hour ago' },
		{ avatar: { src: 'https://i.pravatar.cc/150?u=emma', alt: 'Emma' }, title: 'Emma created issue #38', description: 'Bug: Timeline not rendering correctly', date: '2 hours ago' }
	]
...
<div class="space-y-12">
	<div class="space-y-2">
		...
		<p class="text-on-surface-variant">
			Display a sequence of events with dates, titles, icons or avatars. Supports vertical and horizontal orientations.
		</p>
	</div>
			...
			<div class="space-y-4 rounded-lg border border-outline-variant p-4">
				...
				<div class="space-y-2">
					...
					<div class="flex gap-2">
						<button
							...
							class:bg-surface-container-highest={playgroundOrientation !== 'vertical'}
						...
						<button
							...
							class:bg-surface-container-highest={playgroundOrientation !== 'horizontal'}
				...
				<div class="space-y-2">
					...
					<label class="flex items-center gap-2">
						<input type="checkbox" bind:checked={playgroundReverse} class="accent-primary" />
		...
		<div class="grid gap-6 lg:grid-cols-2">
				...
				<div class="rounded-lg bg-surface-container-high p-4">
					<Timeline
						items={[
							...
							{ value: 3, icon: 'lucide:check-circle', title: 'Approved', date: 'Dec 8' }
						]}
						value={2}
						color="success"
					/>
			...
			<div class="space-y-2">
				...
				<div class="rounded-lg bg-surface-container-high p-4">
					<Timeline
						items={[
							{ avatar: { src: 'https://i.pravatar.cc/150?u=a', alt: 'Alice' }, title: 'Alice started', date: '2h ago' },
							{ avatar: { src: 'https://i.pravatar.cc/150?u=b', alt: 'Bob' }, title: 'Bob reviewed', date: '1h ago' },
		...
		<div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
			{#each colors as color}
				<div class="rounded-lg bg-surface-container-high p-3">
					<p class="mb-2 text-xs font-medium capitalize text-on-surface-variant">{color}</p>
					<Timeline
						items={[
							{ value: 1, title: 'Done' },
	...
	<section class="space-y-4">
		...
		<div class="overflow-x-auto">
			<div class="flex gap-6 rounded-lg bg-surface-container-high p-4" style="min-width: max-content;">
				{#each sizes as size}
	...
	<section class="space-y-6">
		...
		<div class="space-y-3">
			...
			<div class="rounded-lg border border-outline-variant bg-surface-container p-4">
				...
				<div class="mt-4 flex items-center justify-center gap-2">
					<Button size="sm" variant="outline" onclick={() => orderStep = Math.max(1, orderStep - 1)} disabled={orderStep === 1}>
						Previous
					</Button>
					<span class="px-3 text-sm text-on-surface-variant">Step {orderStep} of 4</span>
					<Button size="sm" variant="outline" onclick={() => orderStep = Math.min(4, orderStep + 1)} disabled={orderStep === 4}>
						Next
					</Button>
				</div>
			</div>
		...
		<div class="space-y-3">
			...
			<div class="rounded-lg border border-outline-variant bg-surface-container p-4">
				<Timeline items={checkoutItems} value={checkoutStep} orientation="horizontal" size="lg" />
				<div class="mt-4 flex items-center justify-between">
					<Button size="sm" variant="ghost" onclick={() => checkoutStep = Math.max(1, checkoutStep - 1)} disabled={checkoutStep === 1}>
						Back
					</Button>
					<Button size="sm" onclick={() => checkoutStep = Math.min(4, checkoutStep + 1)} disabled={checkoutStep === 4}>
	...
	<section class="space-y-6">
			...
			<div class="rounded-lg bg-surface-container-high p-4">
				...
				>
					{#snippet content({ item, state })}
						<div class="mt-2 rounded-lg border border-outline-variant bg-surface-container p-3">
							{#if item.value === 'v1'}
		...
		<div class="space-y-3">
			...
			<div class="rounded-lg bg-surface-container-high p-4">
				<Timeline
					items={[
						{ value: 1, icon: 'lucide:star', title: 'Styled indicator', date: 'Custom ring' },
						{ value: 2, icon: 'lucide:zap', title: 'Gradient separator', date: 'Colorful' },
						{ value: 3, icon: 'lucide:heart', title: 'Bold title', date: 'Italic date' }
					]}
					value={2}
					ui={{
						indicator: 'ring-2 ring-primary ring-offset-2 ring-offset-surface-container-high',
