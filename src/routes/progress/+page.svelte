...
<div class="space-y-8">
	...
	<section class="space-y-3">
		...
		<div class="space-y-4 rounded-lg bg-surface-container-high p-4">
			...
			<div class="flex items-center gap-2">
				<Button size="sm" variant="outline" onclick={() => (value = Math.max(0, value - 10))}>
					-10
				</Button>
				<input
					type="range"
					...
					class="flex-1"
				/>
				<Button size="sm" variant="outline" onclick={() => (value = Math.min(100, value + 10))}>
					+10
				</Button>
			</div>
		</div>
	...
	<section class="space-y-3">
		...
		<p class="text-sm text-on-surface-variant">
			Customize the status display with a snippet.
		</p>
		<div class="space-y-4 rounded-lg bg-surface-container-high p-4">
			<Progress value={75} status>
				{#snippet statusSlot({ percent })}
					<span class="text-primary font-medium">{percent}% completed</span>
				{/snippet}
			</Progress>
			...
			<table class="w-full">
				<thead>
					<tr class="border-b border-outline-variant">
						...
						{#each sizes as size}
							<th class="px-3 py-3 text-center text-sm font-medium text-on-surface-variant">{size}</th>
						{/each}
					</tr>
				</thead>
				<tbody>
	...
	<section class="space-y-3">
		...
		<div class="space-y-6 rounded-lg bg-surface-container-high p-4">
			...
			<div class="space-y-2">
				...
				<div class="rounded-lg border border-outline-variant bg-surface-container p-4">
					<Progress value={2} max={['Cart', 'Shipping', 'Payment', 'Done']} color="success" />
				</div>
			</div>
			...
			<div class="space-y-2">
				...
				<div class="space-y-3 rounded-lg border border-outline-variant bg-surface-container p-4">
					<div class="space-y-1">
