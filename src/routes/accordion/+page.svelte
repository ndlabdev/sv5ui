<script lang="ts">
	...
	const iconItems: AccordionItem[] = [
		{
			...
			icon: 'lucide:user',
			content: 'Manage your profile information, email preferences, and account security settings.'
		},
		{
			label: 'Notifications',
			icon: 'lucide:bell',
			content: 'Configure push notifications, email alerts, and in-app notification preferences.'
		},
		{
	...
	const trailingItems: AccordionItem[] = [
		...
		{ label: 'Delete Item', content: 'Click to delete this item.', trailingIcon: 'lucide:trash-2' }
	]
...
<div class="space-y-8">
	<div class="space-y-2">
		...
		<p class="text-on-surface-variant">
			A vertically stacked set of interactive headings that reveal or hide associated content. Built
			on top of bits-ui Accordion.
		</p>
	</div>
	...
	<section class="space-y-3">
		...
		<p class="text-sm text-on-surface-variant">
			Pass an array of items with <code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">label</code> and
	...
	<section class="space-y-3">
		...
		<p class="text-sm text-on-surface-variant">
			Use <code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">type="single"</code>
	...
	<section class="space-y-3">
		...
		<p class="text-sm text-on-surface-variant">
			Override the default chevron with <code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">trailingIcon</code>
	...
	<section class="space-y-3">
		...
		<p class="text-sm text-on-surface-variant">Disable the entire accordion or individual items.</p>
		<div class="grid gap-4 md:grid-cols-2">
			<div class="rounded-lg bg-surface-container-high p-4">
	...
	<section class="space-y-3">
		...
		<p class="text-sm text-on-surface-variant">
			Bind the <code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">value</code>
	...
	<section class="space-y-3">
		...
		<p class="text-sm text-on-surface-variant">
			Assign custom <code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">value</code>
		...
		<div class="rounded-lg bg-surface-container-high p-4">
			<Accordion
				...
				items={[
					...
					{ label: 'Getting Started', content: "Let's begin with the basics.", value: 'start' },
					{
	...
	<section class="space-y-3">
		...
		<p class="text-sm text-on-surface-variant">
			Set <code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">loop</code> to cycle
			keyboard focus at boundaries.
		</p>
		<div class="rounded-lg bg-surface-container-high p-4">
	...
	<section class="space-y-3">
		...
		<p class="text-sm text-on-surface-variant">
			Set <code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">orientation</code>
	...
	<section class="space-y-3">
		...
		<p class="text-sm text-on-surface-variant">
			Use <code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">forceMount</code>
		...
		<div class="grid gap-4 md:grid-cols-2">
			<div class="rounded-lg bg-surface-container-high p-4">
				...
				<p class="mb-3 text-xs text-on-surface-variant">Content is removed from DOM when closed</p>
				<Accordion type="single" items={basicItems} />
			</div>
			<div class="rounded-lg bg-surface-container-high p-4">
				<p class="mb-3 text-sm font-medium">Force mount enabled</p>
				<p class="mb-3 text-xs text-on-surface-variant">Content stays in DOM (inspect element)</p>
	...
	<section class="space-y-3">
		...
		<p class="text-sm text-on-surface-variant">
			Use snippets for custom rendering: <code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">leading</code>,
					...
					{#snippet trailing({ item, open })}
						<div class="flex items-center gap-2">
							{#if item.value === 'enterprise'}
								<Badge size="xs" variant="outline" color="warning" label="Contact us" />
							{/if}
							<Icon
					...
					{#snippet body({ item })}
						<div class="flex items-start gap-3">
							...
							<div>
								...
								<Button size="xs" variant="link" label="Learn more" class="mt-2 h-auto p-0" />
							</div>
						</div>
					{/snippet}
				</Accordion>
	...
	<section class="space-y-3">
		...
		<p class="text-sm text-on-surface-variant">
			Override slot styles via the <code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">ui</code>
	...
	<section class="space-y-3">
		...
		<p class="text-sm text-on-surface-variant">
			Each item can have its own <code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">ui</code>
			and
				...
				{#snippet body({ item })}
					<div class="flex justify-end gap-8 text-center text-sm">
						{#if item.value === 'storage'}
							...
							<div class="w-16 text-primary font-medium">100 GB</div>
							<div class="w-16">Unlimited</div>
						{:else if item.value === 'users'}
							<div class="w-16">1</div>
							<div class="w-16 text-primary font-medium">10</div>
							<div class="w-16">Unlimited</div>
						{:else}
							<div class="w-16">Email</div>
							<div class="w-16 text-primary font-medium">Priority</div>
	...
	<section class="space-y-3">
		...
		<p class="text-sm text-on-surface-variant">
			Use the <code class="rounded bg-surface-container-highest px-1.5 py-0.5 text-xs">body</code> or
			...
			>
				{#snippet body({ item })}
						...
						<div class="flex items-center gap-4 rounded-lg bg-surface-container p-4">
							...
							<div class="flex-1">
								...
								<div class="mt-2 flex gap-2">
									<Badge size="xs" variant="soft" color="success" label="Active" />
									<Badge size="xs" variant="outline" color="primary" label="Pro Plan" />
								</div>
							</div>
					...
					{:else if item.value === 'activity'}
						...
						<div class="space-y-3">
							{#each [
								{ time: '2 min ago', text: 'Updated profile picture', icon: 'lucide:image', color: 'text-primary' },
								{ time: '1 hour ago', text: 'Completed task #123', icon: 'lucide:check-circle', color: 'text-success' },
								{ time: '3 hours ago', text: 'Added new comment', icon: 'lucide:message-circle', color: 'text-info' }
							] as activity}
								<div class="flex items-center gap-3">
									<div class="flex size-8 shrink-0 items-center justify-center rounded-full bg-surface-container">
										<Icon name={activity.icon} size="16" class={activity.color} />
									</div>
									<div class="flex-1">
					...
					{:else if item.value === 'actions'}
						...
						<div class="grid grid-cols-2 gap-2">
							<Button variant="outline" leadingIcon="lucide:plus" label="New Item" class="justify-start" />
							<Button variant="outline" leadingIcon="lucide:upload" label="Upload" class="justify-start" />
							<Button variant="outline" leadingIcon="lucide:download" label="Export" class="justify-start" />
							<Button variant="outline" leadingIcon="lucide:share-2" label="Share" class="justify-start" />
						</div>
					{/if}
				{/snippet}
			</Accordion>
	...
	<section class="space-y-3">
		...
		<p class="text-sm text-on-surface-variant">
			Combine different content types like forms, images, code blocks, and interactive elements.
		</p>
		<div class="rounded-lg bg-surface-container-high p-4">
			<Accordion
				type="multiple"
				...
				{#snippet body({ item })}
					{#if item.value === 'form'}
						...
						<div class="space-y-3">
							...
							<Button variant="solid" label="Send Message" leadingIcon="lucide:send" />
						</div>
					{:else if item.value === 'gallery'}
						...
						<div class="grid grid-cols-3 gap-2">
							{#each Array(6) as _, i}
								...
								>
									<Icon name="lucide:image" size="24" class="text-on-surface-variant/50" />
								</div>
							{/each}
						</div>
						<p class="mt-2 text-center text-xs text-on-surface-variant">Click to view full size</p>
					{:else if item.value === 'code'}
						<!-- Code Block UI -->
						<div class="overflow-hidden rounded-lg bg-surface-container-highest">
							<div class="flex items-center justify-between border-b border-outline-variant/30 px-3 py-2">
								<span class="text-xs text-on-surface-variant">accordion.svelte</span>
								<Button size="xs" variant="ghost" leadingIcon="lucide:copy" label="Copy" />
							</div>
							<pre class="overflow-x-auto p-3 text-xs"><code class="text-on-surface-variant">&lt;Accordion
  type="single"
  items=&#123;items&#125;
  bind:value
&gt;
