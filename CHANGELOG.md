...
- **Tree** — hierarchical tree view (custom build): bindable `value`/`expanded`, `multiple` selection with `propagateSelect`/`bubbleSelect` and indeterminate state, full keyboard navigation, `api` handle for expand/collapse/select, custom item snippets. ([#178](https://github.com/ndlabdev/sv5ui/pull/178))
- **Lightbox** — full-screen media viewer (bits-ui Dialog): image/video/iframe slides, zoom and pan, swipe and keyboard navigation, thumbnail filmstrip, captions, slideshow, fullscreen and rotate, bindable `open`/`index` plus `bind:api`. ([#177](https://github.com/ndlabdev/sv5ui/pull/177))
