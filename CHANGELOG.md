...
- **Rating** — star rating input (bits-ui): bindable `value` with `onValueChange`, `max`/`min`, `allowHalf` half-star steps (clipped-overlay rendering), `readonly` and `hoverPreview`, slider a11y semantics (arrow keys, `aria-valuenow`/customizable `aria-valuetext`), hidden input for form submission when `name` is set, FormField integration (error color, aria wiring), customizable `icon` (new `icons.star` config entry) with a `fill` prop (auto-fill stroke icons when active, or `false` for outline-only) and an optional `activeIcon` for genuinely filled variants, 8 colors, 5 sizes.
