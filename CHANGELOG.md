...
- **InputNumber** — numeric input with increment/decrement steppers (custom build, no Bits UI primitive): locale-aware formatting and parsing via `@internationalized/number` (`formatOptions` for currency/percent/unit/decimals, `locale`), bindable `value` (`number | null`) that tracks typing live and clamps to `min`/`max` on commit (blur/Enter), stepper operations snapped to `step` multiples (`stepSnapping` opt-out), press-and-hold repeat on steppers, ArrowUp/Down, Home/End, and focused mouse-wheel stepping (`disableWheelChange`, `invertWheelChange`), horizontal or vertical stepper `orientation`, steppers rendered as real `Button`s customizable via `increment`/`decrement` (ButtonProps) plus `incrementSlot`/`decrementSlot` content overrides, `role="spinbutton"` semantics, hidden input for native form submission, FormField integration (error color, aria wiring), Input-style variants (`outline`/`soft`/`subtle`/`ghost`/`none`), and full `ui` slot overrides.
- **InputTags** — tag entry input rendering values as removable `Badge`s (custom build, no Bits UI primitive): bindable `value` (`string[]`), commit via Enter, inline `delimiter` typing, delimiter/newline-splitting paste (`addOnPaste`), `addOnTab`/`addOnBlur`, duplicate rejection (`allowDuplicates` opt-in), `max` tag count and `maxLength` per tag, two-step Backspace deletion with ArrowLeft/ArrowRight tag navigation and Delete/Escape handling, per-tag delete buttons (`deleteIcon`, `deleteAriaLabel`), tags customizable via `tag` (BadgeProps) plus a `tagSlot` content override, leading/trailing icon, avatar, and slot support shared with Input, one hidden input per tag for native form submission (`FormData.getAll`), FormField integration (error color, aria wiring), Input-style variants with `focus-within` field styling, and full `ui` slot overrides.
- **TimeField** — segmented time input built on the bits-ui TimeField primitive: bindable `value`/`placeholder` accepting `Time`, `CalendarDateTime`, or `ZonedDateTime` (only the time portion is edited), `granularity` (`hour`/`minute`/`second`), `hourCycle` (12/24), locale-aware segments with per-segment spinbutton keyboard editing, `minValue`/`maxValue` and `validate`/`onInvalid` constraints, `readonlySegments`, trailing clock `icon` (customizable, `icon={false}` reclaims the padding), hidden input for native form submission (ISO time string), FormField integration (error color, aria wiring), Input-style variants (`outline`/`soft`/`subtle`/`ghost`/`none`), and full `ui` slot overrides.
...
- **DatePicker** — new `timeInput` prop renders a segmented time field below the calendar (with `timeIcon`, `time`/`timeIcon`/`timeField` ui slots), implies `granularity: 'minute'` when unset, and flips the `closeOnDateSelect` default to `false` so the popover stays open for time entry; editing the time before picking a date anchors the date portion to `placeholder`.
- **DateRangePicker** — new `timeInput` prop renders start and end time fields below the range calendar with the same defaults and ui slots as DatePicker (`closeOnRangeSelect` defaults to `false` when set).
